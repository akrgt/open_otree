...
doc = """
二種類の最終提案ゲーム：10期繰り返し直接応答法とストラテジーメソッド
...
class Constants(BaseConstants):
    name_in_url = 'repeat_ultimatum'
    players_per_group = 2
    num_rounds = 10

    instructions_template = 'repeat_ultimatum/Instructions.html'
...
class Group(BaseGroup):
    strategy = models.BooleanField(
        doc="""このグループがストラテジーメソッドを使うかどうか／Whether this group uses strategy method"""
    )

    amount_offered = models.CurrencyField(choices=Constants.offer_choices)

    offer_accepted = models.BooleanField(
        doc="このオファーを受けるかどうか／if offered amount is accepted (direct response method)"
    )
